...
//тип ошибка содержит код шттп и ошибку
type ApiError struct {
	HTTPStatus int
	Err        error
}
//метод к ошибке вывоит строку ошибки
func (ae ApiError) Error() string {
	return ae.Err.Error()
}

// ----------------
//объявление констант для статуса пользователя, статуса модератора и админа
const (
	statusUser      = 0
	statusModerator = 10
	statusAdmin     = 20
)
///тип мой апи имеет статусы пользователей нэкст айди и мьютекс!!!
type MyApi struct {
