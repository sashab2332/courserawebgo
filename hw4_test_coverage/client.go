...
func (srv *SearchClient) FindUsers(req SearchRequest) (*SearchResponse, error) {
	...
	case http.StatusInternalServerError://статус ошибка сервера
		return nil, fmt.Errorf("SearchServer fatal error")
	case http.StatusBadRequest://статус плохой реквест
		errResp := SearchErrorResponse{}//присваивание структуры ошибки ответа
		err = json.Unmarshal(body, &errResp)//расщифравка файла
		//fmt.Println(errResp)
		if err != nil {
			//fmt.Println("!!")
